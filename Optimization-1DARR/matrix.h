...
public:
    ...
    void print();
    void populate();
    int &get(int row, int col) { return grid[rowCol2Index(row,col)]; }
    int rowCol2Index(int row, int col) { return N * row + col; }
    int searchCost() { return grid[rowCol2Index(0, N - 1)]; }
    string findRoot();
    ~Matrix();
