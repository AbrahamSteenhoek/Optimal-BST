...
void Matrix::populate() {
    for (size_t diag = 1; diag < N; diag++) {
        for(size_t i = 0; i < N - diag; i++) {
            // first value
            int min = grid[rowCol2Index(i + 1, diag + i)];
            
            for(size_t k = 1; k < diag; k++) {
                min = std::min(min, grid[rowCol2Index(i, i + k - 1)] + grid[rowCol2Index(i + k + 1, diag + i)]);
            }

            // last value
            min = std::min(min, grid[rowCol2Index(i, diag + i - 1)]);
            ...
            grid[rowCol2Index(i, diag + i)] = min;
            // print();
        }
    }
}
...
string Matrix::findRoot() {
    vector<int> vals;
    int diag = N - 1;
    int min = grid[rowCol2Index(0, N - 1)] - runningSum[N - 1];
    vals.push_back(grid[rowCol2Index(1, diag)]);

    for(size_t k = 1; k < diag; k++) {
        vals.push_back(grid[rowCol2Index(0, k - 1)] + grid[rowCol2Index(k + 1, diag)]);
    }

    vals.push_back(grid[rowCol2Index(0, diag - 1)]);
...
void Matrix::print() {
    for (int i = 0; i < N; i++) {
        for(int j = 0; j < N; j++) {
            if(grid[rowCol2Index(i,j)] > 100) {
                cout << " ";
            }
            else if(grid[rowCol2Index(i,j)] >= 10) {
                cout << "  ";
            }
            else if(grid[rowCol2Index(i,j)] >= 0) {
                cout << "   ";
            }
            cout << grid[rowCol2Index(i,j)];
        }
        cout << endl;
    }
    // cout << endl;
